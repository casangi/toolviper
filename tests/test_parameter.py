...
from toolviper.utils.parameter import validate, get_path, set_config_directory, is_notebook, verify
...
def test_validate_decorator_success(tmp_path, monkeypatch):
    ...
    with patch("toolviper.utils.parameter.get_path", return_value=(str(tmp_path), str(tmp_path / "test_mod"))):
...
def test_verify_missing_config():
    ...
    with patch("toolviper.utils.parameter.get_path", return_value=("/tmp", "/tmp/ghost_module")), \
         patch("toolviper.utils.logger.error"):
        with pytest.raises(FileNotFoundError):
            verify(no_config_func, {}, {"function": "no_config_func", "module": "ghost_module"})
...
def test_verify_function_not_in_schema(tmp_path):
    ...
    with patch("toolviper.utils.parameter.get_path", return_value=(str(tmp_path), str(tmp_path / "known_mod"))):
        with pytest.raises(KeyError):
            verify(unknown_func, {}, {"function": "unknown_func", "module": "known_mod"}, config_dir=str(config_dir))
